...
def exfat_mkfs(stream, size, sector=512, params={}):
    ...
    if type(stream) == disk.partition:
        # exFAT stores PartitionOffset in sectors, while disk.partition.offset
        # is expressed in bytes. FATtools 1.1.23 wrote the byte offset here.
        boot.u64PartOffset = stream.offset//sector
    else:
